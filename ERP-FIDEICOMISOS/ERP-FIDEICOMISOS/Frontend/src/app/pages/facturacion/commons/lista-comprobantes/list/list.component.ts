...
export class ListComponent implements OnInit, OnChanges {
  ...
  contraComprobante(comprobante: any) {
    this.findAsientoContable(comprobante);
  }

   findAsientoContable(comprobante: any) {
    let referenciaExterna = "UUID-" + comprobante.numero_comprobante;
    this.asientosService.getAsientoByReferencia(referenciaExterna).subscribe({
      next: (response: any) => {
        ...
        if (response.response.response[0].conciliacion) {
          ...
          this.buildContracomprobante(comprobante);
        }
      },
  ...
  buildContracomprobante(comprobante: any) {
    let preComprobante = {
